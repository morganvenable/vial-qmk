...
// Termination action when a configured trigger fires (pointing motion / keypress).
typedef enum {
    PVS_TERM_OFF  = 0,  // trigger does nothing
    PVS_TERM_PARK = 1,  // pvs_park() — soft stop, can resume by re-engaging scroll ball
    PVS_TERM_EXIT = 2,  // pvs_deactivate() + clear toggle/hold flags in keymap_support
} pvs_term_action_t;
...
typedef struct __attribute__((__packed__)) {
    ...
    uint8_t pointing_term_action;   // pvs_term_action_t — fires when pointing trackball moves
    uint8_t gamma_index;            // 0=linear, 1=quadratic, 2=cubic (default 1)
    uint8_t keypress_term_action;   // pvs_term_action_t — fires on any non-PVS keypress
} pvs_config_t;
...
#define PVS_DEFAULT_POINTING_TERM_ACTION  PVS_TERM_PARK
#define PVS_DEFAULT_GAMMA_INDEX           1
#define PVS_DEFAULT_KEYPRESS_TERM_ACTION  PVS_TERM_PARK
#define PVS_DEFAULT_CONFIG { \
    ...
    PVS_DEFAULT_POINTING_TERM_ACTION, \
    PVS_DEFAULT_GAMMA_INDEX, \
    PVS_DEFAULT_KEYPRESS_TERM_ACTION \
}

// Public API
void pvs_activate(uint16_t current_dpi);
void pvs_deactivate(void);
void pvs_park(void);              // soft-stop: zero velocity, stay armed
